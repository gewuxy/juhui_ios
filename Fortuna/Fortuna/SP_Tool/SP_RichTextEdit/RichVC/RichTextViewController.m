...
- (IBAction)backClick:(UIButton *)sender {
    if (self.textView.attributedText.length == 0) {
        ...
        UIAlertController * alertVC=[UIAlertController alertControllerWithTitle:@"退出编辑" message:@"" preferredStyle:UIAlertControllerStyleAlert];
...
- (IBAction)boldClick:(UIButton *)sender {
    ...
    if (self.isBold) {
        sender.titleLabel.font=[UIFont boldSystemFontOfSize:25];
    }
    else
    {
...
- (IBAction)fontClick:(UIButton *)sender {
    ...
    if (sender.selected) {
        self.font=25.f;
        sender.titleLabel.font = [UIFont boldSystemFontOfSize:self.font];
...
-(void)setImageText:(UIImage *)img withRange:(NSRange)range appenReturn:(BOOL)appen
{
    ...
    imageTextAttachment.imageTag = [NSString stringWithFormat:@"%@_%@",RICHTEXT_IMAGE,@"http://"];
