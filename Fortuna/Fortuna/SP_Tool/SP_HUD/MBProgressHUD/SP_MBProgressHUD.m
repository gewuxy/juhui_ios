...
@implementation SP_MBProgressHUD {
	BOOL useAnimation;
...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	SP_MBProgressHUD *hud = [[self alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	SP_MBProgressHUD *hud = [self HUDForView:view];
	if (hud != nil) {
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    if (view == nil) view = [[UIApplication sharedApplication].windows lastObject];
	NSArray *huds = [SP_MBProgressHUD allHUDsForView:view];
	for (SP_MBProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
+ (MB_INSTANCETYPE)HUDForView:(UIView *)view {
	...
	for (UIView *subview in subviewsEnum) {
		if ([subview isKindOfClass:self]) {
			return (SP_MBProgressHUD *)subview;
		}
	}
	return nil;
}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.animationType = SP_MBProgressHUDAnimationFade;
		self.mode = SP_MBProgressHUDModeIndeterminate;
		self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
	if (animated && animationType == SP_MBProgressHUDAnimationZoomIn) {
		self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
	} else if (animated && animationType == SP_MBProgressHUDAnimationZoomOut) {
	...
	if (animated) {
		...
		self.alpha = 1.0f;
		if (animationType == SP_MBProgressHUDAnimationZoomIn || animationType == SP_MBProgressHUDAnimationZoomOut) {
...
- (void)hideUsingAnimation:(BOOL)animated {
	...
	if (animated && showStarted) {
		...
		// in the done method
		if (animationType == SP_MBProgressHUDAnimationZoomIn) {
			self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(1.5f, 1.5f));
		} else if (animationType == SP_MBProgressHUDAnimationZoomOut) {
			self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
		}
...
- (void)updateIndicators {
	...
	if (mode == SP_MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
	...
	else if (mode == SP_MBProgressHUDModeDeterminateHorizontalBar) {
		// Update to bar determinate indicator
		[indicator removeFromSuperview];
        self.indicator = MB_AUTORELEASE([[MBBarProgressView alloc] init]);
		[self addSubview:indicator];
	}
	else if (mode == SP_MBProgressHUDModeDeterminate || mode == SP_MBProgressHUDModeAnnularDeterminate) {
		if (!isRoundIndicator) {
		...
		if (mode == SP_MBProgressHUDModeAnnularDeterminate) {
			[(MBRoundProgressView *)indicator setAnnular:YES];
		}
	} 
	else if (mode == SP_MBProgressHUDModeCustomView && customView != indicator) {
		// Update custom view indicator
		[indicator removeFromSuperview];
		self.indicator = customView;
		[self addSubview:indicator];
	} else if (mode == SP_MBProgressHUDModeText) {
		[indicator removeFromSuperview];
		self.indicator = nil;
	}
}
