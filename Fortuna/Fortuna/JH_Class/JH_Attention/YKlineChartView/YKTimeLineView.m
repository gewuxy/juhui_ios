...
//        self.minPrice =((YKTimeLineEntity *)[self.dataset.data firstObject]).preClosePx - self.offsetMaxPrice;
        self.minTime = ((YKTimeLineEntity *)[self.dataset.data firstObject]).currtTime;
        self.maxTime = ((YKTimeLineEntity *)[self.dataset.data lastObject]).currtTime;
...
//        self.offsetMaxPrice = CGFLOAT_MIN;
        ...
        NSLog(@"self.maxPrice==>%f",self.maxPrice);
        NSLog(@"self.minPrice==>%f",self.minPrice);
        NSLog(@"self.maxVolume==>%f",self.maxVolume);
        NSLog(@"self.offsetMaxPrice==>%f",self.offsetMaxPrice);
...
- (void)drawTimeLabel:(CGContextRef)context
{
    ...
    NSMutableAttributedString * startTimeAttStr = [[NSMutableAttributedString alloc]initWithString:mintime attributes:drawAttributes];
    ...
    NSMutableAttributedString * midTimeAttStr = [[NSMutableAttributedString alloc]initWithString:@"" attributes:drawAttributes];
    ...
    NSString * maxtime = @"";
    if (self.maxTime != nil) {
        mintime = self.maxTime;
    }
    NSMutableAttributedString * stopTimeAttStr = [[NSMutableAttributedString alloc]initWithString:maxtime attributes:drawAttributes];
...
//
//    ;
    //self.frame.size.width = recognizer.scale * self.frame.size.width;
//
