...
class My_Page: SP_ParentVC {

    //MARK:--- 导航栏元素
    fileprivate lazy var naviTitleView: UIView = {
        let view = UIView(frame:CGRect(x: 0, y: 0, width: 150, height: 40))
        return view
    }()
    fileprivate lazy var scrollView_top: UIScrollView = {
        let scrollView = UIScrollView()
        scrollView.bounces = false
        scrollView.showsVerticalScrollIndicator = false
        scrollView.showsHorizontalScrollIndicator = false
        self.naviTitleView.addSubview(scrollView)
        scrollView.snp.makeConstraints { (make) in
            make.edges.equalToSuperview()
        }
        return scrollView
    }()
    fileprivate lazy var scrollView_conter: UIScrollView = {
        let scrollView = UIScrollView()
        scrollView.bounces = false
    ...
    lazy var _lineView:UIView = {
        let view = UIView(frame:CGRect(x:0,y:39,width:50,height:1))
        view.backgroundColor = UIColor.main_1
        return view
    }()
    
    var _addControllerBlock:((_ index:Int)->UIViewController)?
    var _showContentViewBlock:((_ index:Int)->Void)?
    /// 内容区域scrollView x轴偏移量
    var _contentOffsetX: CGFloat = 0.0
    var _itemMargin:CGFloat = 15
    var _lineViewW:CGFloat = 40
    var _lineViewH:CGFloat = 2
    var _labSelectedColor = UIColor.mainText_2
    var _labNormalColor = UIColor.mainText_1
    // 栏目数组
    var _selectIndex = 0
    var selectedArray:[M_My_Page] = [] {
        didSet{
            makePageUI()
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.1) {
                //跳到指定标签
                for item in self.scrollView_top.subviews {
                    if let titleLabel = item as? SP_PageItemLabel, titleLabel.tag == self._selectIndex {
                        guard let tap = titleLabel.gestureRecognizers?.first as? UITapGestureRecognizer else{return}
                        self.didTappedTopLabel(tap)
...
extension My_Page {
//    override class func initSPVC() -> My_Page {
//        return UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "My_Page") as! My_Page
//    }
    ...
    fileprivate func makeNavigation() {
        n_view._title = ""
        n_view.n_view_Title.addSubview(naviTitleView)
...
extension My_Page {
    ...
    fileprivate func addController(_ index:Int) {
        ...
        if let newsVc = self._addControllerBlock?(index) {
...
extension My_Page {
    ...
    private func addContent() {
        ...
        for (i,item) in selectedArray.enumerated() {
            ...
            // 添加标签点击手势
            label.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(My_Page.didTappedTopLabel(_:))))
            //添加控制器
            addController(i)
...
extension My_Page: UIScrollViewDelegate {
    // 滚动结束后触发 代码导致
